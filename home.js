...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateAllBubbles() {
    ...
    bubbles.forEach((bubble) => {
      ...
      if (distance < maxDistance) {
        ...
        bubble.style.transform = `translate(${moveX + joltX}px, ${
          moveY + joltY
        }px)`;
      } else {
  ...
  for (let i = 0; i < 15; i++) {
    ...
    setTimeout(() => {
      ...
      if (Math.random() < 0.4) {
        // Increased chance from 0.3 to 0.4
        setTimeout(createBubble, Math.random() * 300);
        if (Math.random() < 0.3) {
          // Additional chance for a third bubble
  ...
  setInterval(() => {
    ...
    if (Math.random() < 0.3) {
      // Increased chance from 0.2 to 0.3
      setTimeout(createBubble, Math.random() * 300);
      if (Math.random() < 0.2) {
        // Chance for a third bubble
        setTimeout(createBubble, Math.random() * 400);
      }
    }
